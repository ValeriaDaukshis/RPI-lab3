...
function getAngleBetweenVectors(x1, y1, x2, y2) { 
	var skalar = x1 * x2 + y1*y2;
	var vector1 = Math.sqrt(x1*x1+y1*y1);
	var vector2 = Math.sqrt(x2*x2+y2*y2);
	return Math.acos(skalar / (vector1 * vector2));
}
...
function parseNumberFromString(value) {
    return parseFloat(value,10); // дописываем с/с 
}
